...
# # # pip install google-generativeai streamlit Pillow PyPDF2 cantera
...
# # import base64
# # import os
# # import cantera as ct # New import for Cantera
...
# # # --- Cantera Tool Definitions (NO @genai.tool decorator) ---
# # # These are now just regular Python functions that the app will call manually.
...
# #     """
# #     try:
# #         gas = ct.Solution('gri30.yaml') # Using a common mechanism for combustion
        
# #         # Set initial state
# #         gas.TP = initial_temp_k, initial_pressure_pa

# #         # Define reactants based on fuel and oxidizer
# #         if oxidizer.lower() == 'air':
# #             gas.set_equivalence_ratio(equivalence_ratio, fuel, {'O2': 1.0, 'N2': 3.76})
# #         else: # Assume pure oxidizer if not 'air'
# #             gas.set_equivalence_ratio(equivalence_ratio, fuel, oxidizer)
        
# #         # Perform adiabatic flame temperature calculation
# #         gas.equilibrate('HP')
...
# #     """
# #     try:
# #         gas = ct.Solution('gri30.yaml') # Load a common mechanism to access species properties
...
# #             "molecular_weight_g_mol": mw / 1000.0 # Convert to g/mol
# #         })
# #     except Exception as e:
# #         return json.dumps({
# #             "status": "error",
...
# #     """
# #     try:
# #         gas = ct.Solution('gri30.yaml') # Load a common mechanism
# #         gas.TPX = temperature_k, pressure_pa, mixture_formula
# #         gas.equilibrate('TP') # Equilibrate at constant T and P
...
# # # This dictionary allows us to call the functions dynamically by their name.
# # available_tools = {
...
# #     "get_equilibrium_concentrations": get_equilibrium_concentrations # NEW TOOL ADDED
# # }


# # # --- Initialize the Generative Model (NO tools argument here) ---
# # model = genai.GenerativeModel("gemini-1.5-flash")

# # # --- System Instruction Prompt (UPDATED for Manual Tool Use) ---
# # system_instruction_prompt = """
# # You are Catalyst Mind, a chatbot powered by Google Generative AI, specializing in chemical operations and process control.
# # Your core expertise lies in explaining chemical processes, providing insights into control strategies,
# # answering questions about chemical engineering principles, and interpreting relevant diagrams/documents.

# # **You have access to powerful tools to perform chemical calculations. When a calculation is required, you MUST output a JSON object in the following format, followed by a brief confirmation message (e.g., "Executing calculation...") or a question if parameters are missing:**

# # ```json
# # {
# #   "tool_call": {
# #     "name": "tool_function_name_here",
# #     "args": {
# #       "param1": "value1",
# #       "param2": "value2"
# #     }
...
# # - **`calculate_adiabatic_flame_temperature`**: To determine adiabatic flame temperature.
# #     - `fuel` (string, e.g., 'CH4', 'C2H5OH')
# #     - `oxidizer` (string, e.g., 'O2', 'air')
# #     - `equivalence_ratio` (float, e.g., 1.0 for stoichiometric, 0.8 for lean)
# #     - `initial_temp_k` (float, temperature in Kelvin, e.g., 298.15)
# #     - `initial_pressure_pa` (float, pressure in Pascals, e.g., 101325.0)
# # - **`get_species_molecular_weight`**: To find the molecular weight of a species.
# #     - `species_name` (string, e.g., 'CO2', 'H2O', 'CH4')
# # - **`get_equilibrium_concentrations`**: To calculate equilibrium mole fractions of a mixture.
# #     - `mixture_formula` (string, initial composition, e.g., 'CH4:1, O2:2, N2:7.52')
# #     - `temperature_k` (float, equilibrium temperature in Kelvin, e.g., 1500.0)
# #     - `pressure_pa` (float, equilibrium pressure in Pascals, e.g., 101325.0)

# # **Your workflow for user requests:**
# # 1.  **Understand the User's Goal**: Identify if the user is asking for information, a calculation, or both.
# # 2.  **Tool Application (Agentic Behavior)**:
# #     * If the user asks for a calculation that matches one of your tools, you **MUST** output the tool call JSON as specified above.
# #     * **Always provide all necessary parameters** in the JSON `args`. If a parameter is missing, ask the user for clarification within your response *before* outputting the JSON.
# #     * **After outputting a tool call JSON, you will receive the tool's output.** Interpret these results and present them clearly to the user, explaining what the numbers mean in the context of their query.
# #     * **If a tool call result indicates an error**, inform the user about the failure and suggest rephrasing or providing more information.
# # 3.  **Information Retrieval**: If the user asks for general information, provide it directly from your knowledge base.
# # 4.  **Multimodal Input Handling**:
# #     * If an image is uploaded, analyze it in the context of chemical operations or process control.
# #     * If a PDF or TXT file is uploaded, I will extract text from it and include it in your query; use this extracted text to inform your response.
# # 5.  **Adherence to Domain**: Your domain/area of expertise is strictly limited to chemical operations and process control. You are not to provide any information outside of this domain. If you are unable to answer a question or the question falls outside of the specified domain, kindly inform the user.
# # 6.  **Safety First**: Ensure that any query or request that can significantly harm human life should be avoided. If necessary, ask questions for clarification based on the user's request.
...
# # pip install google-generativeai streamlit Pillow PyPDF2 cantera numpy
...
# # --- API Key Configuration ---
# api_key = "AIzaSyBCjtvQ7kvYQcImqRToLSDyfkofCGiRM74"  # Replace with your actual API key

# if not api_key:
#     st.error("Google Generative AI API key is not set.")
#     st.stop()
...
# # --- Cantera Tool Functions ---
# def calculate_adiabatic_flame_temperature(fuel: str, oxidizer: str, equivalence_ratio: float, 
#                                         initial_temp_k: float, initial_pressure_pa: float) -> dict:
...
#             "temperature_C": float(gas.T - 273.15),
#             "species_concentrations": {sp: float(conc) for sp, conc in zip(gas.species_names, gas.X)}
#         }
#     except Exception as e:
#         return {"status": "error", "message": str(e)}

# def get_equilibrium_concentrations(mixture: str, temperature_k: float, pressure_pa: float) -> dict:
#     """Calculate equilibrium concentrations using Cantera."""
#     try:
#         gas = ct.Solution('gri30.yaml')
...
#         return {
#             "status": "success",
#             "mole_fractions": {sp: float(conc) for sp, conc in zip(gas.species_names, gas.X)},
#             "temperature_K": float(gas.T),
...
# def get_species_properties(species_name: str, temperature_k: float, pressure_pa: float) -> dict:
#     """Get thermodynamic properties of a species using Cantera."""
#     try:
#         gas = ct.Solution('gri30.yaml')
#         if species_name not in gas.species_names:
#             return {"status": "error", "message": f"Species '{species_name}' not found in mechanism"}
            
#         gas.TPX = temperature_k, pressure_pa, {species_name: 1.0}
#         return {
#             "status": "success",
#             "species": species_name,
#             "molecular_weight_kg_kmol": float(gas.molecular_weights[gas.species_index(species_name)]),
#             "enthalpy_J_kmol": float(gas.enthalpy_mole),
#             "entropy_J_kmolK": float(gas.entropy_mole),
#             "gibbs_J_kmol": float(gas.gibbs_mole),
#             "cp_J_kmolK": float(gas.cp_mole)
#         }
#     except Exception as e:
#         return {"status": "error", "message": str(e)}
...
#     "calculate_adiabatic_flame_temperature": calculate_adiabatic_flame_temperature,
#     "get_equilibrium_concentrations": get_equilibrium_concentrations,
#     "get_species_properties": get_species_properties
# }
...
# def extract_tool_call(response_text: str) -> tuple:
...
#                 return tool_call["tool_call"], response_text[json_end:].strip()
#     except json.JSONDecodeError:
#         pass
